...
%Závěrem lze konstatovat, že evidence a kompenzace hmotnosti nalévačů by mohla být funkční pouze v případě, že by podnik standardizoval typ nalévačů napříč sortimentem. Tato strategie však nebyla reálná, neboť provoz preferoval flexibilní nákup dle aktuálních cenových a skladových možností. Z těchto důvodů byla databáze nalévačů i s příslušnou funkcionalitou odstraněna z finální verze systému.
...
%\begin{lstlisting}[language=Python, caption=Metoda turn\_on\_sensor, frame=single, breaklines=true, postbreak=\mbox{\textcolor{gray}{$\hookrightarrow$}\space}]
%
