...
\section{Mikrokontrolér} %Mikrokontroler
%Výpočetní modul / Modul s výpočetní jednotkou / Řídící jednotka

%Výpočetní modul je samostatná hardwarová součást (typicky deska plošných spojů s procesorem, pamětí a nezbytnými rozhraními), která zajišťuje hlavní výpočetní výkon ve větším elektronickém nebo řídicím systému. Pracují často s dalšími moduly, ať už řídícími(mikrokontroler, jednodeskový počítač, atd), sběrnicovými, vstupními(senzory, klávesnice, atd) či výstupními(diplej, reproduktor, atd)
...
\subsection{Požadavky na řídící jednotku / výpočetní modul}
%napájení
...
Hlavním z požadavků je snadná realizovatelnost firmware s integrovaným grafickým uživatelským rozhraním(GUI) ovládaného dotykovým displejem. Pro takové účely je vhodné použít vysokoúrovňový programovací jazyk, např. Python, který nabízí široký výběr knihoven a nástrojů, které ulehčují a zrychlují vývoj samotného firmware, na druhou stranu je nutné přihlédnout na dostačující výkon a paměť řídící jednotky.
Firmware poběží na operačním systému Linux.

Další požadavky:
\begin{itemize}
    \item Výkon: Vyvíjená GUI aplikace bude zobrazována a ovládána vybraným displejem[5.5.2] s rozlišením 1024x600 a současně zpracovávat data ze čtečky čárového kódu[5.6.2] a váhy[5.4.2], pro tyto účely by se hodil dvou a více jádrový procesor... grafický akcelerátor. Další z věcí, která by mohla ulehčit chod aplikace, by byl grafický akcelerátor.
    \item Výkon: Dvou a více jádrový procesor pro paralelní chod GUI a stavového automatu pro zpracování dat ze čtečky čárového kódu a váhy.
    \item Periferie:
        \begin{itemize}
            \item 3x USB: Čtečka čárového kódu, váha, napájení displeje 
            \item 1x HDMI: Dotykový displej
            \item slot na SD kartu na které budou uložena veškerá data databáze zmíněná v kapitole č. 5.3, operační systém a samotný firmware.
        \end{itemize}
        3x USB a 1x HDMI nebo Micro HDMI port pro připojení jednotlivých komponent měřicího systému - váha, EAN čtečka, displej.
    \item Další požadavek je integrovaný slot pro SD kartu na které budou uložena veškerá data databáze zmíněná v kapitole č. 5.3, operační systém a samotný firmware.
    \item Wi-Fi modul: Do budoucna je cílem bezdrátová konektivita s měřícím systémem pomocí mobilní aplikace nebo připojení se k ERP systémům
\end{itemize}
