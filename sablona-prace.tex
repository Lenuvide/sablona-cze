...
\include{text/1) Inventura}
\include{text/2) Dosavadní metody měření}
\include{text/3) Výpočet objemu}
\include{text/4) Sériová komunikace}
\include{text/5) Nově navržený systém}
\include{text/6) Zprovoznění jednotlivých komponent}
\include{text/7) Vývoj firmwaru}
\include{text/8) Testování}
\include{text/9) Inovace}
